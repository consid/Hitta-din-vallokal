...
namespace ValdagGeoCodeApp
{
    ...
    {
        /// <summary>
        /// Reads the vallokal för valdagen excelfile and get geographical coordinates from Google maps
        ...
        /// The output file will be used as a source file for the "VallokalerValdagen" table in Azure storage
        /// </summary>
        static void Main(string[] args)
        {
            ...
            using (var connection = new OleDbConnection(connectionString))
            {
                ...
                using (var dr = command.ExecuteReader())
                {
                    while (dr != null && dr.Read())
                    {
                        ...
                        data.valdistriktId = dr.GetValue(0).ToString();
                        data.valdistriktNamn = dr.GetValue(1).ToString();
                        data.vallokalNamn = dr.GetValue(2).ToString();
                        data.adress = dr.GetValue(3).ToString();
                        data.postNr = dr.GetValue(4).ToString();
                        data.postOrt = dr.GetValue(5).ToString();
                        resultList.Add(data);
                    }
                }
            }
            ...
            //Loop the list and supply the geographical data and also supply a clean adress 
            ...
            using (var connection = new OleDbConnection(connectionString_write))
            {
                ...
                {
                    var query = "INSERT INTO" + "[" + worksheet + "]" + "([Valdistrikt_id],[Valdistrikt_namn],[Vallokal_namn],[Adress],[Postnr],[Postort],[Latitud],[Longitud],[Adress_geo]) " + "VALUES(@value1, @value2, @value3, @value4, @value5, @value6, @value7, @value8, @value9)";

                    var command = new OleDbCommand(query, connection);
                    command.Parameters.AddWithValue("@value1", resultList[j].valdistriktId);
                    command.Parameters.AddWithValue("@value2", resultList[j].valdistriktNamn);
                    command.Parameters.AddWithValue("@value3", resultList[j].vallokalNamn);
                    command.Parameters.AddWithValue("@value4", resultList[j].adress);
                    command.Parameters.AddWithValue("@value5", resultList[j].postNr);
                    command.Parameters.AddWithValue("@value6", resultList[j].postOrt);
                    command.Parameters.AddWithValue("@value7", resultList[j].latitud);
                    command.Parameters.AddWithValue("@value8", resultList[j].longitud);
                    command.Parameters.AddWithValue("@value9", resultList[j].adress_geo);
    ...
    class ResultData
    {
        public string valdistriktId { get; set; }
